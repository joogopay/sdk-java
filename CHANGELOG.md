...
- Local validation before signing: top-level required fields, decimal-string
  ...
  the gateway.
- Error taxonomy: `JoogopayException.Request` (rejected before sending), `JoogopayException.Transport` (sent or possibly sent, no response), `JoogopayException.Api` (gateway business error), `JoogopayException.Response` (non-envelope response).
